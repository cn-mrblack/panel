...
const ButtonStyle = styled.button<Omit<Props, 'isLoading'>>`
    ${tw`relative inline-block rounded-lg px-4 py-2 uppercase tracking-wide text-sm transition-all duration-300 border`};

    ${(props) =>
        ((!props.isSecondary && !props.color) || props.color === 'primary') &&
        css<Props>`
            ${(props) => !props.isSecondary && tw`bg-gradient-to-r from-purple-500 to-cyan-500 border-transparent text-white`};

            &:hover:not(:disabled) {
                ${tw`from-purple-600 to-cyan-600 shadow-glow`};
            }
        `};
    ...
    ${(props) =>
        ...
        css<Props>`
            ...
            &:hover:not(:disabled) {
                ...
                ${(props) => props.color === 'primary' && tw`bg-gradient-to-r from-purple-500 to-cyan-500 border-transparent text-white shadow-glow`};
